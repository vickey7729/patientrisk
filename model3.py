...
class PatientRiskModel2:
    def __init__(self):
        ...
        self.clinical_rules = {
            ...
            ('high_bp', 'heart_failure'): 12,
            ('high_fever', 'pneumonia'): 10,
            ('high_bp', 'diabetes'): 8,
            ('low_bp', 'falls'): 10
        }
    ...
    def preprocess_features(self, data: Dict) -> pd.DataFrame:
        ...
        features['systolic_bp'] = vitals.get('systolic_bp', 120)
        features['diastolic_bp'] = vitals.get('diastolic_bp', 80)
        features['high_bp'] = int(features['systolic_bp'] >= 140 or features['diastolic_bp'] >= 90)
        features['low_bp'] = int(features['systolic_bp'] < 90 or features['diastolic_bp'] < 60)
        
        # Handle temperature
        features['temperature'] = vitals.get('temperature', 98.6)
        features['high_fever'] = int(features['temperature'] >= 100.4)
        features['low_temp'] = int(features['temperature'] < 97.0)
    ...
    def _prepare_training_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Blood pressure (extract from vital signs if available)
        # Since BP isn't directly in the dataset, we'll add placeholder features that will be filled during prediction
        features['systolic_bp'] = 120  # Default normal value
        features['diastolic_bp'] = 80  # Default normal value
        features['high_bp'] = 0
        features['low_bp'] = 0
        
        # Temperature (extract from vital signs if available)
        # Since temperature isn't directly in the dataset, we'll add placeholder features
        features['temperature'] = 98.6  # Default normal value in Fahrenheit
        features['high_fever'] = 0
        features['low_temp'] = 0
    ...
    def _apply_clinical_rules(self, patient_data: Dict) -> float:
        ...
        # Extract vitals for BP and temperature
        ...
        condition_dict = {
            ...
            'age>70': age > 70,
            'age>80': age > 80,
            'high_bp': systolic_bp >= 140 or diastolic_bp >= 90,
            'low_bp': systolic_bp < 90 or diastolic_bp < 60,
            'high_fever': temperature >= 100.4,
            'low_temp': temperature < 97.0
        }
        ...
        # BP-specific rules
        if systolic_bp >= 180 or diastolic_bp >= 110:  # Stage 3 hypertension
            score += 12
        elif systolic_bp < 80 or diastolic_bp < 50:  # Severe hypotension
            score += 15
            
        # Temperature-specific rules
        if temperature >= 103:  # High fever
            score += 12
        elif temperature < 95:  # Hypothermia
            score += 18
    ...
    def _calculate_confidence(self, patient_data: Dict) -> float:
        ...
        critical_fields = [
            ...
            ('vitals', ['bmi', 'systolic_bp', 'diastolic_bp', 'temperature']),
            ('medications', [])
        ]
    ...
    def _get_factor_description(self, feature_name: str) -> str:
        ...
        elif feature_name == 'high_bp':
            return "High Blood Pressure"
        elif feature_name == 'low_bp':
            return "Low Blood Pressure"
        elif feature_name == 'high_fever':
            return "Elevated Temperature"
        elif feature_name == 'low_temp':
    ...
    def _get_detailed_description(self, feature: str, importance: float) -> str:
        ...
        descriptions = {
            ...
            'high_bp': "High blood pressure increases cardiovascular risk",
            'low_bp': "Low blood pressure may indicate serious underlying conditions",
            'high_fever': "Elevated temperature indicates possible infection or inflammation",
            'low_temp': "Low body temperature can indicate poor circulation or systemic issues",
            'systolic_bp': "Systolic blood pressure is an important cardiovascular health indicator",
            'diastolic_bp': "Diastolic blood pressure is linked to cardiac workload",
            'temperature': "Body temperature is a key vital sign for overall health assessment"
        }
    ...
    def validate(self, validation_df: pd.DataFrame) -> Dict:
        ...
        for _, row in validation_df.iterrows():
            ...
            patient_data = {
                ...
                'conditions': {'chronic_conditions': []},
                'vitals': {
                    'systolic_bp': 120,  # Default values since not in dataset
                    'diastolic_bp': 80,
                    'temperature': 98.6
                }
            }
            ...
            # Handle BMI if available
            if 'BMI' in validation_df.columns:
                patient_data['vitals']['bmi'] = row.get('BMI', 0)
